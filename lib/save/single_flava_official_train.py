...
import SMART_official.flava_data_loader as dl
...
def train(args, dataloader, im_backbone):
    ...
    def train_loop(epoch, train_loader, optimizer, train_loss_dict, valid_acc_dict):
        model.train()
        tot_loss = 0.0
        ...
        for i, (im, qa, opts, lbl, av, pids) in tqdm(enumerate(train_loader)):
                        
            save_idx = (epoch*len(train_loader)+i)
            
            if args.no_meta:
                out = model(im, qa, puzzle_ids=pids)
            ...
            if i % log_freq == 0:
                ...
                print(f'\n\nLog*******Epoch: {epoch} Train batch: {i+1}/{len(train_loader)}*******\n')
                print(f'Pids: {pids}')
    ...
    def val_loop(val_loader, model):
        ...
        puzzle_acc = {}
        with torch.no_grad():
            for i, (im, qa, o, a, av, pids) in tqdm(enumerate(val_loader)):
    ...
    if args.optimizer == "adam":
        optimizer = torch.optim.Adam(parameters, lr=args.lr, betas=(0.9, 0.99), weight_decay=0.9)
        if not args.no_meta:
            anshead_optimizer = torch.optim.Adam(anshead_parameters, lr=args.lr, betas=(0.9, 0.99), weight_decay=0.9)
    else:
    ...
    for epoch in range(args.num_epochs):
        tt = time.time()
        model.train()
        loss, train_loss_dict, valid_acc_dict = train_loop(epoch, train_loader, optimizer, train_loss_dict, valid_acc_dict)
        tt = time.time() - tt
