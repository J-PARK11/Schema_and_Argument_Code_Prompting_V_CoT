...
def train(args, dataloader, im_backbone):
    ...
    def train_loop(epoch, train_loader, optimizer):
        ...
        for i, (batch_input, im_path, q, _, a, av, pids) in tqdm(enumerate(train_loader)):
            ...
            if i % log_freq == 0:
                ...
                train_loss_dict[i] = save_loss
                print(f'\n\nLog*******batch: {i+1}/{len(train_loader)}*******\n')
                print(f'Pids: {pids}')
            ...
            if i % validation_freq == 0:
                ...
                valid_acc_dict[i] = {"S_acc": float(acc), "O_acc": float(oacc)}
    ...
    for epoch in range(args.num_epochs):
        tt = time.time()
        model.train()
        loss, train_loss_dict, valid_acc_dict = train_loop(epoch, train_loader, optimizer)
        tt = time.time() - tt
