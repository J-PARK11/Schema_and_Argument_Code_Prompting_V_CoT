...
# @dataclass
# class Qwen2VLClassficationOutputWithPast(ModelOutput):
#     """
...
class Qwen2VLofficiaclassification(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.vocab_size = config.vocab_size
        self.hidden_emb = config.hidden_size
        self.max_val = 256 + 1
        
        self.args_use_single_image_head=False
        self.args_monolithic = False
        
        self.args_puzzles = 'all'
        self.args_puzzle_ids = ['66', '27', '28', '35', '69', '85', '51', '2', '30', '58', '34', '59', '39', '29', '87', '100', '22', '31', '81', '44', '55', '4', '65', '79', '42', '75', '83', '60', '18', '17', '47', '89', '99', '91', '61', '92', '37', '57', '32', '72', '93', '41', '43', '96', '10', '21', '88', '12', '36', '19', '3', '77', '90', '71', '25', '5', '13', '7', '49', '26', '24', '84', '33', '48', '82', '38', '70', '1', '73', '67', '101', '98', '40', '76', '52', '64', '54', '9', '15', '53', '6', '94', '62', '16', '8', '80', '45', '56', '97', '78', '74', '14', '86', '46', '50', '23', '68', '63', '20', '11', '95']
        self.sorted_puzzle_ids = np.sort(np.array([int(ii) for ii in self.args_puzzle_ids]))
        self.args_loss_type = "classifier"
        ...
        if self.args_monolithic:
            self.qvo_fusion = nn.Sequential(nn.Linear(self.out_dim, self.max_val))
        else:
            self.create_puzzle_tail()
            
        self.pool = nn.AdaptiveMaxPool1d(1)
        self.lm_head = nn.Linear(1536, self.out_dim * 2)
    ...
    def create_puzzle_tail(self):

        ans_decoder = [
            nn.Sequential(nn.Linear(self.out_dim, 1))
        ]  # start with a dummy as we are 1-indexed wrt puzzle ids.
        if self.args_puzzles == "all":
            puzzles = range(1, gv.num_puzzles + 1)
        else:
            puzzles = self.args_puzzle_ids
        for pid in puzzles:  # self.puzzle_ids:
            num_classes = gv.NUM_CLASSES_PER_PUZZLE[str(pid)] if self.args_loss_type == "classifier" else 1
    ...
    def forward(
        ...
        video_grid_thw: Optional[torch.LongTensor] = None,
        rope_deltas: Optional[torch.LongTensor] = None,
        puzzle_ids: Optional[torch.LongTensor] = None,
        ...
        hidden_states = outputs['last_hidden_state'].transpose(1,2)    # [B, input_ids_len, hidden_emb]
        hidden_states = self.pool(hidden_states).squeeze(-1)
        hidden_states = self.lm_head(hidden_states)
        
        qv_feat = self.qv_fusion(hidden_states)

        if self.args_monolithic:
            qv_feat = qv_feat.unsqueeze(1)
            qvo_feat = self.qvo_fusion(qv_feat).squeeze(1)
        else:
