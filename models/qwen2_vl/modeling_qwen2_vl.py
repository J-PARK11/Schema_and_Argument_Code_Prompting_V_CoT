...
class Qwen2VLForConditionalClassfication(Qwen2VLPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]
    ...
    def __init__(self, config):
        ...
        self.pred_head = nn.Sequential(
                ...
                nn.ReLU(),
                # nn.BatchNorm1d(self.hidden_emb),
                nn.Dropout(0.3),
                nn.Linear(self.hidden_emb, self.hidden_emb//2),
                nn.ReLU(),
                # nn.BatchNorm1d(self.hidden_emb//2),
                nn.Dropout(0.3),
                nn.Linear(self.hidden_emb//2, self.hidden_emb//2),
                nn.ReLU(),
                # nn.BatchNorm1d(self.hidden_emb//2),
                nn.Dropout(0.3),
                nn.Linear(self.hidden_emb//2, self.max_val))
        
        self.pool = nn.AdaptiveMaxPool1d(1)
        self.pred_softmax = nn.Softmax(-1)
        self.padding_side = "left"  # set it to left by default, user can use setter to change padding_sides
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
    ...
    def forward(
        ...
        hidden_states = outputs['last_hidden_state']    # [B, input_ids_len, hidden_emb]
        hidden_states = self.pred_head(hidden_states)   # [B, input_ids_len, max_val]
        hidden_states = hidden_states.transpose(1,2)    # [B, max_val, input_ids_len]
        logits = self.pool(hidden_states).squeeze(-1)   # [B, max_val]
        logits = self.pred_softmax(logits)              # [B, max_val]
        logits = logits.float()                         
